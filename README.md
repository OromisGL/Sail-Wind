...
# Sail-Wind
#### Video Demo:  [<URL HERE>](https://youtu.be/VVBgUBGAt0Y)
...
The web app makes use of Flask for backend routing and templating, and MongoDB for flexible and scalable data storage. With Folium integrated into the app, users can view sailing routes on interactive maps, where wind speed is represented through color-coded paths. Real-time weather data is fetched from the German Weather Service, providing users with up-to-date information on wind conditions. The app also leverages GPX data for visualizing GPS routes and uses Flask-Login for user authentication. By combining threading for background tasks and Bootstrap for responsive design, the app offers a dynamic and user-friendly experience.
